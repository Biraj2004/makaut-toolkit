...
export default function CalculatorsPage() {
  ...
  const [marksResult, setMarksResult] = useState<{ total: number, obtained: number, percentage: number } | null>(null);
  ...
  const handleCalculateDgpa = () => {
    const requiredInputs = dgpaType === "2yr" ? 2 : dgpaType === "lateral" || dgpaType === "3yr" ? 3 : 4;
  ...
  const handleCalculateCgpa = () => {
    ...
    // Check for incomplete rows among filled ones could be complex, 
    // but simplified: Just check if any filled row has parsing error? 
    // Or strictly check if we have matching pairs?
    // Let's filter for complete pairs.
    const validPairs = cgpaData.filter(d => d.sgpa !== "" && d.credits !== "");
  ...
  const handleCgpaInputChange = (index: number, field: "sgpa" | "credits", value: string) => {
    // Determine max value based on field
    // sgpa: max 10
    // credits: strict < 50 (so, max check 49.999 effectively < 50)
    // For credits, let's just use < 50 check manually + decimal check
  ...
  const renderDgpaInputs = () => {
    let count = 0;
    switch (dgpaType) {
        case "4yr": count = 4; break;
        case "lateral": count = 3; break;
        case "3yr": count = 3; break;
        case "2yr": count = 2; break;
    }
  ...
  const renderCgpaInputs = () => {
    let semesterCount = 0;
    switch (cgpaType) {
        case "2yr": semesterCount = 4; break;
        case "3yr": semesterCount = 6; break;
        case "4yr": semesterCount = 8; break;
        case "5yr": semesterCount = 10; break;
    }
            ...
            <div className="space-y-2">
                <Label className="text-xs text-zinc-400">Sem {i + 1} Credits</Label>
                 <Input
                    ...
                    onChange={(e) => handleCgpaInputChange(i, "credits", e.target.value)}
                />
            </div>
        </div>
    ));
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="space-y-2 text-center md:text-left">
        ...
        <p className="text-zinc-400">
            Essential tools for managing your grades, checking <span className="text-emerald-400 font-medium">SVMCM Scholarship eligibility</span>, and academic performance.
        </p>
      </div>

      <Tabs defaultValue="sgpa-percent" className="w-full" onValueChange={() => setError(null)}>
        <TabsList className="grid w-full grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 bg-zinc-900 h-auto p-1">
          <TabsTrigger value="sgpa-percent" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-emerald-500 py-2 cursor-pointer">SGPA to %</TabsTrigger>
          <TabsTrigger value="ygpa-percent" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-emerald-500 py-2 cursor-pointer">YGPA to %</TabsTrigger>
          <TabsTrigger value="yearly-marks" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-emerald-500 py-2 cursor-pointer">Marks Calc</TabsTrigger>
          <TabsTrigger value="dgpa-calc" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-emerald-500 py-2 cursor-pointer">DGPA Calc</TabsTrigger>
          <TabsTrigger value="cgpa-calc" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-emerald-500 py-2 cursor-pointer">CGPA Calc</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="sgpa-percent">
          <Card className="bg-zinc-900 border-zinc-800 mt-4">
            <CardHeader>
              ...
              <CardDescription>Convert your Semester Grade Point Average (SGPA) to percentage.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="rounded-lg bg-zinc-950/50 border border-zinc-800 p-4">
                  <span className="font-semibold text-zinc-400 block mb-1 text-sm">Formula used:</span>
                  <code className="text-zinc-500 text-sm">Percentage = (SGPA - 0.75) × 10</code>
              </div>
              ...
              {sgpaResult !== null && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-md p-4 flex items-center gap-3 text-emerald-500 animate-in fade-in slide-in-from-top-2">
                  ...
                  <span className="font-medium">Equivalent Percentage: {formatResult(sgpaResult)}%</span>
                </div>
              )}
            </CardContent>
            <CardFooter className="justify-end gap-3 border-t border-zinc-800 pt-6">
                <Button variant="outline" onClick={handleClear} className="border-zinc-700 hover:bg-zinc-800 hover:text-white cursor-pointer">
                    <RotateCcw className="w-4 h-4 mr-2" /> Clear
                </Button>
                <Button onClick={handleCalculateSgpa} className="bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer">Calculate</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="yearly-marks">
            <Card className="bg-zinc-900 border-zinc-800 mt-4">
                <CardHeader>
                    ...
                    <CardDescription>Calculate SGPA to Total Marks, Obtained Marks & Percentage.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                     <div className="space-y-4">
                        <div className="text-sm text-zinc-400 leading-relaxed">
                            <p className="mb-2">This Tool is helpful for each year student to calculate their marks Percentage for Scholarship Form Fill Up at the time of Renewal, and Eligibility Check.</p>
                            ...
                            <div className="mt-4 p-4 bg-zinc-950/50 rounded-md border border-zinc-800 text-sm text-zinc-400 leading-relaxed shadow-sm space-y-2">
                                <div className="font-semibold text-zinc-200">Yearly Marks Calculation for Scholarships:</div>
                                <p>
                                    Enter your <span className="text-zinc-300 font-medium">Odd & Even Semester SGPA</span> and <span className="text-zinc-300 font-medium">Number of Subjects</span> (Theory + Practical) to calculate your:
                                </p>
                        ...
                        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-md p-4 space-y-2 animate-in fade-in slide-in-from-top-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div className="bg-zinc-950/50 p-3 rounded border border-emerald-500/10">
                                    <span className="text-zinc-400 block text-xs uppercase tracking-wider mb-1">Total Marks</span>
                                    <span className="text-xl font-bold text-white">{marksResult.total}</span>
                                </div>
                                <div className="bg-zinc-900/50 p-3 rounded border border-emerald-500/10">
                                    <span className="text-zinc-400 block text-xs uppercase tracking-wider mb-1">Obtained Marks</span>
                                    <span className="text-xl font-bold text-emerald-400">{formatResult(marksResult.obtained)}</span>
                                </div>
                                <div className="bg-zinc-900/50 p-3 rounded border border-emerald-500/10">
                                    <span className="text-zinc-400 block text-xs uppercase tracking-wider mb-1">Percentage</span>
                                    <span className="text-xl font-bold text-emerald-400">{formatResult(marksResult.percentage)}%</span>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <CardFooter className="justify-end gap-3 border-t border-zinc-800 pt-6">
                    <Button variant="outline" onClick={handleClear} className="border-zinc-700 hover:bg-zinc-800 hover:text-white cursor-pointer"><RotateCcw className="w-4 h-4 mr-2" /> Clear</Button>
                    <Button onClick={handleCalculateMarks} className="bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer">Calculate</Button>
                </CardFooter>
            </Card>
        </TabsContent>
        ...
        <TabsContent value="ygpa-percent">
          <Card className="bg-zinc-900 border-zinc-800 mt-4">
            <CardHeader>
              ...
              <CardDescription>Calculate annual percentage from odd and even semester SGPAs.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="rounded-lg bg-zinc-950/50 border border-zinc-800 p-4 space-y-2">
                  <span className="font-semibold text-zinc-400 block text-sm">Formulas used:</span>
                  <div className="flex flex-col gap-1">
                    <code className="text-zinc-500 text-sm">YGPA = (Odd SGPA + Even SGPA) / 2</code>
                    <code className="text-zinc-500 text-sm">Percentage = (YGPA - 0.75) × 10</code>
                  </div>
              </div>
              ...
              {ygpaResult !== null && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-md p-4 space-y-3 animate-in fade-in slide-in-from-top-2">
                  <div className="flex items-center gap-3 text-emerald-500">
                    <CheckCircle2 className="w-5 h-5 shrink-0" />
                    <div>
                        <span className="font-medium block">Yearly Average YGPA: {avgYgpa !== null ? formatResult(avgYgpa) : ''}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 text-emerald-500 border-t border-emerald-500/20 pt-3">
                    <CheckCircle2 className="w-5 h-5 shrink-0" />
                    <span className="font-medium">Yearly Percentage: {formatResult(ygpaResult)}%</span>
                  </div>
                </div>
              )}
            </CardContent>
             <CardFooter className="justify-end gap-3 border-t border-zinc-800 pt-6">
                <Button variant="outline" onClick={handleClear} className="border-zinc-700 hover:bg-zinc-800 hover:text-white cursor-pointer"><RotateCcw className="w-4 h-4 mr-2" /> Clear</Button>
                <Button onClick={handleCalculateYgpa} className="bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer">Calculate</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="dgpa-calc">
            ...
            <CardHeader>
              ...
              <CardDescription>Calculate your final Degree GPA based on your programme type.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                    <Label>Course Type</Label>
                    <Select value={dgpaType} onValueChange={(val) => handleDgpaTypeChange(val as DegreeType)}>
                        <SelectTrigger className="bg-zinc-950 border-zinc-700 cursor-pointer">
                            <SelectValue placeholder="Select Course Type" />
                        </SelectTrigger>
                        <SelectContent className="bg-zinc-950 border-zinc-800 text-white">
                            <SelectItem className="cursor-pointer" value="4yr">4 Year Degree (B.Tech)</SelectItem>
                            <SelectItem className="cursor-pointer" value="lateral">Lateral Entry (3 Years)</SelectItem>
                            <SelectItem className="cursor-pointer" value="3yr">3 Year Degree</SelectItem>
                            <SelectItem className="cursor-pointer" value="2yr">2 Year Degree (M.Tech/MCA/MBA)</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
                ...
                <div className="rounded-lg bg-zinc-950/50 border border-zinc-800 p-4">
                    ...
                    <code className="text-zinc-500 text-sm">
                        {dgpaType === "4yr" && "DGPA = (YGPA1 + YGPA2 + 1.5*YGPA3 + 1.5*YGPA4) / 5"}
                        {dgpaType === "lateral" && "DGPA = (YGPA2 + 1.5*YGPA3 + 1.5*YGPA4) / 4"}
                ...
                {(dgpaType === "4yr" || dgpaType === "lateral") && (
                    <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3 flex gap-3 items-start text-sm text-amber-500/90">
                        ...
                        <div className="space-y-1">
                            ...
                            {dgpaType === "4yr" && <p>For Regular B.Tech students, the calculation considers all 4 years. 1st and 2nd years carry normal weightage, while 3rd and 4th years carry 1.5x weightage.</p>}
                            {dgpaType === "lateral" && <p>For Lateral Entry students, the calculation starts from the 2nd Year. The 2nd year carries normal weightage, while 3rd and 4th years carry 1.5x weightage.</p>}
                        </div>
                    </div>
                )}
              </div>
              ...
              {dgpaResult !== null && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-md p-4 flex items-center gap-3 text-emerald-500 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="flex flex-col">
                      <span className="font-medium text-lg">DGPA: {formatResult(dgpaResult)}</span>
                      <span className="text-xs opacity-75">Degree Grade Point Average</span>
                  </div>
                </div>
              )}
            </CardContent>
            <CardFooter className="justify-end gap-3 border-t border-zinc-800 pt-6">
                <Button variant="outline" onClick={handleClear} className="border-zinc-700 hover:bg-zinc-800 hover:text-white cursor-pointer"><RotateCcw className="w-4 h-4 mr-2" /> Clear</Button>
                <Button onClick={handleCalculateDgpa} className="bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer">Calculate</Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="cgpa-calc">
            <Card className="bg-zinc-900 border-zinc-800 mt-4">
                <CardHeader>
                    ...
                    <CardDescription>Calculate your Cumulative Grade Point Average (CGPA).</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label>Course Duration</Label>
                            <Select value={cgpaType} onValueChange={(val) => handleCgpaTypeChange(val as CgpaProgramType)}>
                                <SelectTrigger className="bg-zinc-950 border-zinc-700 cursor-pointer">
                                    <SelectValue placeholder="Select Course Duration" />
                                </SelectTrigger>
                                <SelectContent className="bg-zinc-950 border-zinc-800 text-white">
                                    <SelectItem className="cursor-pointer" value="4yr">4 Year (8 Semesters)</SelectItem>
                                    <SelectItem className="cursor-pointer" value="3yr">3 Year (6 Semesters)</SelectItem>
                                    <SelectItem className="cursor-pointer" value="2yr">2 Year (4 Semesters)</SelectItem>
                                    <SelectItem className="cursor-pointer" value="5yr">5 Year (10 Semesters)</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                        ...
                        <div className="rounded-lg bg-zinc-950/50 border border-zinc-800 p-4 space-y-4">
                            <div>
                                <span className="font-semibold text-zinc-400 block mb-1 text-sm">Formula used:</span>
                                <code className="text-zinc-500 text-sm">CGPA = ∑ (Credit Index) / ∑ (Credits)</code>
                            </div>

                            <div className="space-y-3 pt-2 border-t border-zinc-800/50">
                                <h4 className="text-sm font-medium text-zinc-300">Explanation of Terms:</h4>
                                
                                <div className="space-y-1">
                                    <p className="text-xs font-semibold text-emerald-500">1. Credit Index of a Semester</p>
                                    <p className="text-xs text-zinc-500 leading-relaxed">
                                        It is the <span className="text-zinc-400">total credit points</span> earned in that semester.
                                        <br />
                                        Calculated as: <span className="italic">Credit Index = Total Semester Credit Points = SGPA × Total Semester Credits</span>
                                    </p>
                                </div>

                                <div className="space-y-1">
                                    <p className="text-xs font-semibold text-emerald-500">2. Credits of a Semester</p>
                                    <p className="text-xs text-zinc-500 leading-relaxed">
                                        Total number of credits registered in that semester.
                                        <br />
                                        Includes <span className="text-zinc-400">theory + lab + practical credits</span>.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {cgpaResult !== null && (
                        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-md p-4 flex items-center gap-3 text-emerald-500 animate-in fade-in slide-in-from-top-2">
                            <CheckCircle2 className="w-5 h-5 shrink-0" />
                            <div className="flex flex-col">
                                <span className="font-medium text-lg">CGPA: {formatResult(cgpaResult)}</span>
                                <span className="text-xs opacity-75">Cumulative Grade Point Average</span>
                            </div>
                        </div>
                    )}
                </CardContent>
                <CardFooter className="justify-end gap-3 border-t border-zinc-800 pt-6">
                    <Button variant="outline" onClick={handleClear} className="border-zinc-700 hover:bg-zinc-800 hover:text-white cursor-pointer"><RotateCcw className="w-4 h-4 mr-2" /> Clear</Button>
                    <Button onClick={handleCalculateCgpa} className="bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer">Calculate</Button>
                </CardFooter>
            </Card>
        </TabsContent>
      </Tabs>
